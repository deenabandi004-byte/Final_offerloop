# Email Template Previews

This document lists all **20** preview emails shown on the Email Template page (`/contact-search/templates`). Each preview is a sample for the same fictional context:

- **Recipient:** Alex Chen, Software Engineer at Google, San Francisco  
- **Sender:** Sid, CS junior at USC (in the app, the sender name is replaced with the current user’s first name)

Keys are `purpose_style` and match the backend presets in `backend/email_templates.py`.

---

## Networking (5)

### 1. networking_casual

> Hi Alex,
>
> I'm Sid, CS at USC — been geeking out over Google's search infra work lately. Curious what your day-to-day actually looks like on the team. Down for a quick 15-min chat sometime?
>
> Thanks,  
> Sid

---

### 2. networking_professional

> Hi Alex,
>
> My name is Sid, and I'm a junior studying Computer Science at USC. I came across your profile while researching engineering roles at Google, and your work on search infrastructure stood out. Would you be available for a brief 15–20 minute conversation?
>
> Best regards,  
> Sid

---

### 3. networking_short_direct

> Hi Alex,
>
> CS junior at USC. Your search infra work at Google is exactly what I want to do. 15 min to chat about how you got there?
>
> Thanks,  
> Sid

---

### 4. networking_warm_enthusiastic

> Hi Alex,
>
> I'm Sid, a CS student at USC, and I've been genuinely fascinated by the search infrastructure work coming out of Google — the scale of what your team handles is incredible. I'd love to hear what drew you to that side of things. Would you be open to a 15-minute chat?
>
> Really appreciate it,  
> Sid

---

### 5. networking_bold_confident

> Hi Alex,
>
> Most networking emails start with "I hope this finds you well" — so I'll skip that. I'm Sid, CS at USC, diving deep into distributed systems. Google's search stack is the benchmark. What's the gnarliest scaling problem you've tackled there?
>
> 15 min if you're game.
>
> Thanks,  
> Sid

---

## Referral (5)

### 6. referral_casual

> Hi Alex,
>
> I'm Sid, CS at USC — I've been really into the infra work Google's doing and saw there's an open SWE role on the team. Figured I'd shoot my shot. Any chance you'd be open to passing along my resume or pointing me to the right person?
>
> Thanks,  
> Sid

---

### 7. referral_professional

> Hi Alex,
>
> My name is Sid, and I'm a junior studying Computer Science at USC. I noticed an open Software Engineer position at Google that aligns closely with my background in distributed systems. Given your experience on the search infrastructure team, I wanted to ask if you would be willing to refer me or connect me with the hiring team.
>
> I've attached my resume for context. Thank you for considering it.
>
> Best regards,  
> Sid

---

### 8. referral_short_direct

> Hi Alex,
>
> Sid, CS junior at USC. There's an open SWE role at Google that fits my background. Would you be open to referring me? Resume attached.
>
> Thanks,  
> Sid

---

### 9. referral_warm_enthusiastic

> Hi Alex,
>
> I'm Sid, a CS student at USC, and I've been genuinely excited about the engineering work at Google. I noticed there's an open SWE role and your team's search infrastructure work is exactly the kind of problem I want to tackle. Would you be open to referring me or pointing me to the right person?
>
> Really appreciate it,  
> Sid

---

### 10. referral_bold_confident

> Hi Alex,
>
> I'm Sid, CS at USC. I've spent the last year building distributed systems and I want to do it at Google. There's an open SWE role that's a strong fit. I'd rather get a warm intro than cold-apply — would you be willing to refer me?
>
> Resume attached. Thanks,  
> Sid

---

## Follow-Up (5)

### 11. follow_up_casual

> Hi Alex,
>
> Just bumping this — totally get that things are busy. Still would love to chat for 15 min if you get a chance. No rush at all.
>
> Thanks,  
> Sid

---

### 12. follow_up_professional

> Hi Alex,
>
> I wanted to follow up on my previous message. I understand you have a busy schedule, and I remain very interested in learning about your experience at Google. If you have 15–20 minutes in the coming weeks, I would greatly appreciate the opportunity to connect.
>
> Best regards,  
> Sid

---

### 13. follow_up_short_direct

> Hi Alex,
>
> Following up on my note last week. Saw Google just announced the new search features — congrats to the team. Still happy to chat for 15 min if you have time.
>
> Thanks,  
> Sid

---

### 14. follow_up_warm_enthusiastic

> Hi Alex,
>
> Just circling back — I saw Google's latest search update and it reminded me how much I'd love to hear about your work on the team. Totally understand if timing doesn't work right now, but I'm flexible whenever you are!
>
> Would love to chat,  
> Sid

---

### 15. follow_up_bold_confident

> Hi Alex,
>
> Round two. I reached out last week about your search infra work — still think it's some of the most interesting engineering happening right now. 15 minutes, any time that works.
>
> Thanks,  
> Sid

---

## Sales (5)

### 16. sales_casual

> Hi Alex,
>
> I'm Sid — I built Offerloop, a tool that helps students find and reach out to professionals like you (in a non-spammy way, promise). We've got 1,000+ students using it and I think Google's recruiting team might find it useful. Worth a quick chat?
>
> Thanks,  
> Sid

---

### 17. sales_professional

> Hi Alex,
>
> My name is Sid, and I'm the co-founder of Offerloop, a platform that helps university students connect with industry professionals for career development. Over 1,000 students are currently using the tool, and I believe it could complement Google's campus recruiting efforts. Would you have 15 minutes to discuss a potential partnership?
>
> Best regards,  
> Sid

---

### 18. sales_short_direct

> Hi Alex,
>
> Built Offerloop — helps students find and email professionals. 1,000+ users, strong engagement. Could be useful for Google's campus recruiting. 15-min demo?
>
> Thanks,  
> Sid

---

### 19. sales_warm_enthusiastic

> Hi Alex,
>
> I'm Sid, and I built something I'm really proud of — Offerloop helps students connect with professionals for career opportunities, and over 1,000 students are already using it. I think there's a natural fit with what Google does on campus recruiting and I'd love to explore that with you.
>
> Would love to chat,  
> Sid

---

### 20. sales_bold_confident

> Hi Alex,
>
> Most career platforms give students a job board and call it a day. We built Offerloop — it finds the right people at the right companies and writes personalized outreach in seconds. 1,000+ students are using it. Worth a 15-min demo?
>
> Thanks,  
> Sid

---

## Summary

| # | Key | Purpose   | Style              |
|---|-----|-----------|--------------------|
| 1 | networking_casual | Networking | Casual |
| 2 | networking_professional | Networking | Professional |
| 3 | networking_short_direct | Networking | Short & Direct |
| 4 | networking_warm_enthusiastic | Networking | Warm & Enthusiastic |
| 5 | networking_bold_confident | Networking | Bold & Confident |
| 6 | referral_casual | Referral | Casual |
| 7 | referral_professional | Referral | Professional |
| 8 | referral_short_direct | Referral | Short & Direct |
| 9 | referral_warm_enthusiastic | Referral | Warm & Enthusiastic |
| 10 | referral_bold_confident | Referral | Bold & Confident |
| 11 | follow_up_casual | Follow-Up | Casual |
| 12 | follow_up_professional | Follow-Up | Professional |
| 13 | follow_up_short_direct | Follow-Up | Short & Direct |
| 14 | follow_up_warm_enthusiastic | Follow-Up | Warm & Enthusiastic |
| 15 | follow_up_bold_confident | Follow-Up | Bold & Confident |
| 16 | sales_casual | Sales | Casual |
| 17 | sales_professional | Sales | Professional |
| 18 | sales_short_direct | Sales | Short & Direct |
| 19 | sales_warm_enthusiastic | Sales | Warm & Enthusiastic |
| 20 | sales_bold_confident | Sales | Bold & Confident |

**Source:** `connect-grow-hire/src/pages/EmailTemplatesPage.tsx` — `PREVIEWS` map. In the app, "Sid" is replaced with the signed-in user’s first name (fallback: "Your Name").
