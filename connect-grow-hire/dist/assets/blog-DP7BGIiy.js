...
`;function b(n){const t=n.trimStart();if(!t.startsWith("---"))return{data:{},content:n};const a=t.indexOf("---",3);if(a===-1)return{data:{},content:n};const r=t.slice(3,a).trim(),o=t.slice(a+3).trim(),s={},i=r.split(`
`);let e=0;for(;e<i.length;){const d=i[e],l=d.match(/^(\w+):\s*"(.*)"\s*$/);if(l){s[l[1]]=l[2],e++;continue}const m=d.match(/^(\w+):\s*$/);if(m){const g=m[1],h=[];for(e++;e<i.length;){const y=i[e],u=y.match(/^\s+-\s+(\w+):\s*"(.*)"\s*$/);if(u){const p={[u[1]]:u[2]};for(e++;e<i.length;){const c=i[e].match(/^\s{4}(\w+):\s*"(.*)"\s*$/);if(c)p[c[1]]=c[2],e++;else break}h.push(p)}else{if(y.match(/^\w/))break;e++}}s[g]=h;continue}e++}return{data:s,content:o}}const I=Object.assign({"/src/content/blog/cold-email-investment-banking.md":w,"/src/content/blog/cold-email-mckinsey-consultant.md":v,"/src/content/blog/networking-email-templates-college-students.md":k});function T(){var t;const n=[];for(const[a,r]of Object.entries(I)){const{data:o,content:s}=b(r);n.push({title:o.title||"",date:o.date||"",description:o.description||"",slug:o.slug||((t=a.split("/").pop())==null?void 0:t.replace(".md",""))||"",keywords:o.keywords||"",faqSchema:o.faqSchema||void 0,content:s})}return n.sort((a,r)=>new Date(r.date).getTime()-new Date(a.date).getTime()),n}const f=T();function B(){return f}function j(n){return f.find(t=>t.slug===n)}export{j as a,B as g};
