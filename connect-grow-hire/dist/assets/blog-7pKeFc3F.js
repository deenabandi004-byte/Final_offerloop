...
Build your outreach list, start the conversations, and let the compounding begin.`,M=`---
...
The students who land top IB offers are not always the smartest or the most experienced. They are the ones who showed up early, followed up consistently, and treated every conversation as an opportunity to learn and build genuine relationships.`,j=`---
...
`;function H(n){const t=n.trimStart();if(!t.startsWith("---"))return{data:{},content:n};const a=t.indexOf("---",3);if(a===-1)return{data:{},content:n};const r=t.slice(3,a).trim(),o=t.slice(a+3).trim(),s={},i=r.split(`
`);let e=0;for(;e<i.length;){const h=i[e],l=h.match(/^(\w+):\s*"(.*)"\s*$/);if(l){s[l[1]]=l[2],e++;continue}const d=h.match(/^(\w+):\s*$/);if(d){const g=d[1],m=[];for(e++;e<i.length;){const p=i[e],c=p.match(/^\s+-\s+(\w+):\s*"(.*)"\s*$/);if(c){const y={[c[1]]:c[2]};for(e++;e<i.length;){const u=i[e].match(/^\s{4}(\w+):\s*"(.*)"\s*$/);if(u)y[u[1]]=u[2],e++;else break}m.push(y)}else{if(p.match(/^\w/))break;e++}}s[g]=m;continue}e++}return{data:s,content:o}}const N=Object.assign({"/src/content/blog/alumni-networking-guide.md":w,"/src/content/blog/alumni-outreach-email-guide.md":b,"/src/content/blog/coffee-chat-guide.md":k,"/src/content/blog/coffee-chat-questions-to-ask.md":v,"/src/content/blog/cold-email-consulting-recruiting.md":I,"/src/content/blog/cold-email-investment-banking.md":T,"/src/content/blog/cold-email-mckinsey-consultant.md":A,"/src/content/blog/cold-email-tech-internships.md":S,"/src/content/blog/college-networking-statistics-2025.md":C,"/src/content/blog/consulting-recruiting-timeline.md":F,"/src/content/blog/how-to-ask-for-a-coffee-chat.md":M,"/src/content/blog/how-to-find-professional-email-address.md":B,"/src/content/blog/how-to-network-in-college.md":x,"/src/content/blog/investment-banking-recruiting-timeline.md":W,"/src/content/blog/networking-email-templates-college-students.md":j,"/src/content/blog/offerloop-vs-handshake-vs-linkedin-vs-apollo-comparison.md":P,"/src/content/blog/palantir-vs-anduril-recruiting.md":q});function Y(){var t;const n=[];for(const[a,r]of Object.entries(N)){const{data:o,content:s}=H(r);n.push({title:o.title||"",date:o.date||"",description:o.description||"",slug:o.slug||((t=a.split("/").pop())==null?void 0:t.replace(".md",""))||"",keywords:o.keywords||"",faqSchema:o.faqSchema||void 0,content:s})}return n.sort((a,r)=>new Date(r.date).getTime()-new Date(a.date).getTime()),n}const f=Y();function E(){return f}function O(n){return f.find(t=>t.slug===n)}export{O as a,E as g};
