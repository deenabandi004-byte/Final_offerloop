...
import type { CoffeeChatPrepStatus, CoffeeChatPrep, InterviewPrepStatus, InterviewPrep } from "@/services/api";
...
const handleInterviewPrepSubmit = async () => {
  ...
  try {
    ...
    let pollCount = 0;
    const maxPolls = 120; // 6 minutes max (120 * 3s) - comprehensive scraping takes longer
      ...
      const intervalId = setInterval(async () => {
        ...
        try {
          ...
          if (statusResult.status === 'failed' || statusResult.status === 'parsing_failed') {
            clearInterval(intervalId);
            // If parsing failed, show manual input option
          ...
          if (pollCount >= maxPolls) {
            clearInterval(intervalId);
            // Don't reject immediately - check one more time if it's actually done
            setTimeout(async () => {
              try {
...
const downloadInterviewPrepPDF = async (prepId?: string) => {
  ...
  try {
    ...
    try {
      // Try to fetch and download as blob (handles CORS better)
      ...
      const a = document.createElement("a");
      a.href = blobUrl;
      a.download = `interview-prep-${id}.pdf`; // Force download with filename
      a.style.display = "none";
      document.body.appendChild(a);
      a.click();
      ...
      // Fallback: try direct download link if blob fetch fails (CORS issue)
      console.warn("Blob download failed, trying direct link:", fetchError);
      const a = document.createElement("a");
      a.href = pdfUrl;
      a.download = `interview-prep-${id}.pdf`; // Still try to force download
      a.target = "_blank";
      a.rel = "noopener noreferrer";
      a.style.display = "none";
      document.body.appendChild(a);
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                // Reset manual input when URL changes
                          ...
                          {!showManualInput && !jobPostingUrl.trim() && (
                            <div className="space-y-3">
                              <div className="text-sm text-gray-400 mb-2">Or enter manually:</div>
                              <div>
                                <label className="block text-sm font-medium mb-2 text-white">
                                  Company Name
                                </label>
                          ...
                          {parsedJobDetails && (
                            <div className="bg-gray-800/30 backdrop-blur-sm rounded-xl p-4 border border-gray-700/50">
                              <p className="text-sm font-semibold text-white mb-3">Job Details Preview:</p>
                              <div className="space-y-2 text-sm text-gray-300">
                                <div><span className="text-gray-400">Company:</span> {parsedJobDetails.company_name}</div>
                                <div><span className="text-gray-400">Role:</span> {parsedJobDetails.job_title}</div>
                                {parsedJobDetails.level && (
                                  <div><span className="text-gray-400">Level:</span> {parsedJobDetails.level}</div>
                                )}
                                {parsedJobDetails.team_division && (
                                  <div><span className="text-gray-400">Team:</span> {parsedJobDetails.team_division}</div>
                                )}
                                {parsedJobDetails.required_skills && parsedJobDetails.required_skills.length > 0 && (
                                  <div>
                                    <span className="text-gray-400">Key Skills:</span> {parsedJobDetails.required_skills.slice(0, 5).join(', ')}
                          ...
                          <div className="bg-gray-800/30 backdrop-blur-sm rounded-xl p-4 border border-gray-700/50">
                            <p className="text-sm text-gray-300 mb-2">
                              <span className="font-semibold text-white">What you'll receive:</span>
                            </p>
                          ...
                          <Button
                            ...
                            className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-semibold py-6 text-base shadow-lg shadow-purple-500/30 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            {interviewPrepLoading ? (
                              <>
                          ...
                          {interviewPrepStatus === 'completed' && interviewPrepResult && (
                            <div className="space-y-4">
                                ...
                                {interviewPrepResult.insights && (
                                  <div className="space-y-3 text-sm text-gray-200">
                                    {interviewPrepResult.insights.interview_process?.stages && interviewPrepResult.insights.interview_process.stages.length > 0 && (
                                      <div>
                                            <span className="text-gray-400">Interview Stages: </span>
                                    ...
                                    {interviewPrepResult.insights.common_questions && (
                                      <div>
                                        <span className="text-gray-400">Question Categories: </span>
                                        {(() => {
                              ...
                              <Button
                                ...
                                className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-semibold py-6 text-base shadow-lg shadow-purple-500/30"
                              >
