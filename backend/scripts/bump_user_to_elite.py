"""
One-off admin grant: bump a user to elite tier (no Stripe sub).

Usage:
  python backend/scripts/bump_user_to_elite.py <email>
"""
import os
import sys
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python bump_user_to_elite.py <email>")
        sys.exit(1)
    email = sys.argv[1]
    ...
    db.collection('users').document(uid).update({
        ...
        'trialActive': False,
        'manualGrant': True,
        'manualGrantReason': 'admin bump to elite',
