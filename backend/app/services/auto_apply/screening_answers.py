...
- PREFERRED / ADDITIONAL WORK or OFFICE LOCATION(S) — e.g. "preferred office location(s)", "where would you like to work", "select 1-3 locations from below" → answer from the PREFERRED LOCATIONS context (fall back to LOCATION, else the job's own location, else a major hub like "New York, NY"). This OVERRIDES the "none of the options fits → NEEDS_USER" rule: it's a soft preference, not a credential, and the user clicked Auto-apply to get SUBMITTED. If the field lists Options: pick the option(s) that best match the user's geographies, copied VERBATIM — at least ONE (one satisfies "select 1-3"); pick up to the stated max only when several clearly match. If NONE of the user's geographies match the options, pick ONE reasonable offered option verbatim (a real city, not "Other") so the field is satisfied. NEVER NEEDS_USER for a location-preference field.
