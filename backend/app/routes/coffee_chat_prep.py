...
def process_coffee_chat_prep_background(
    ...
    try:
        ...
        # Step 1: Enrich LinkedIn profile. Engine-first (2026-08-31, ported
        # from mobile): the PDL enrich this used to call 401s on the retired
        # key, which killed EVERY prep at this exact line with "check the
        # URL". Coresignal collects the same profile by its LinkedIn
        # shorthand; the warehouse covers people the engine already met; the
        # PDL call stays as a last rung only because it fails in one
        # round-trip.
